...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        `h-8 w-full min-w-0 rounded-none border border-input bg-background px-2.5 py-1 text-base
        shadow-cubist transition-colors outline-none file:inline-flex file:h-6 file:border-0
