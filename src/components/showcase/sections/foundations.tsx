...
const GRADIENT_STOPS = ["start", "mid", "blue", "green", "yellow", "orange", "end"] as const
...
export function FoundationsSection() {
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <DemoBlock
        title="Typography"
        description="Jura, вес 400–700. Примитивы из typography.tsx."
        className="lg:col-span-2"
      >
        <TypographyH1>Дизайн-система</TypographyH1>
        <TypographyLead>
          Пастельный Cubism: острые плоскости, Cubist Shadow и радужный фон.
        </TypographyLead>
        <TypographyH2>Заголовок второго уровня</TypographyH2>
        <TypographyP>
          Основной текст на Jura. Инлайн-код выглядит так:{" "}
        ...
        <TypographyBlockquote>
          Cubism — визуальный язык системы: без скруглений, с объёмом.
        </TypographyBlockquote>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {TOKENS.map((token) => (
            <div key={token.name} className="space-y-2">
              <div className={`h-12 rounded-none shadow-cubist ${token.className}`} aria-hidden />
              <span className="text-muted-foreground font-mono text-xs">{token.name}</span>
            </div>
          ))}
        </div>
      </DemoBlock>
      ...
      >
        <div className="bg-surface-gradient mb-4 h-24 rounded-none border shadow-cubist" />
        <div className="flex flex-wrap gap-4">
          {GRADIENT_STOPS.map((stop) => (
            <code
              key={stop}
              className="bg-muted rounded-none px-2 py-1 font-mono text-xs shadow-cubist"
            >
